...
| F23-LIM-011 | Future Publishing Intelligence depends on provider-supplied sanitized projections and an optional conversational provider; real provider behavior, quality, latency and privacy are not proven by standalone source tests. | Strict allowlists, minimum cohort threshold, sensitive input/output rejection, advisory-only/no-write law, Founder-only permission simulation and provider-unavailable states. | Hostinger tests with real approved providers, representative role/scope checks, privacy payload inspection, provider-failure tests and Founder acceptance. |
