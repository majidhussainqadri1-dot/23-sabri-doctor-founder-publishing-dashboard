<?php
/**
 * Phase 23F policy boundary for File 23-owned cross-module metadata.
 *
 * This class validates request shape and current-user authority. Native object
 * existence, visibility, ownership, and contributor eligibility are rechecked
 * by the runtime service and native-reference resolver.
 *
...
final class SPDB_Collections_Policy {
	...
	public static function collection_statuses(): array {
	...
	public static function target_surfaces(): array {
		return array( 'home', 'news', 'profile', 'learning', 'encyclopedia', 'research', 'video', 'reels', 'pdf', 'clinical_cases', 'remedy_archive', 'disease_archive', 'search' );
	}

	public static function knowledge_relations(): array {
		return array( 'encyclopedia', 'learning', 'research', 'founder_book', 'doctor_knowledge', 'clinical_case', 'remedy_archive', 'disease_archive', 'video_series', 'pdf_series', 'timeline', 'topic_archive', 'search_index' );
	}
	...
	public static function validate_collection( array $input ) {
		...
		$authority = self::mutation_authority( $record_type, $scope );
		if ( is_wp_error( $authority ) ) {
			return $authority;
		}

		$allowed_statuses = 'campaign' === $record_type ? self::campaign_statuses() : self::collection_statuses();
		$values = array(
			...
			'status'              => self::enum_value( $input['status'] ?? 'draft', $allowed_statuses, 'spdb_collection_status_invalid' ),
		...
		if ( self::text_length( $values['audit_reason'] ) < 10 ) {
			return self::error( 'spdb_collection_audit_reason_invalid', 'A meaningful audit reason is required.' );
		}
		if ( '' !== $values['start_at_gmt'] && '' !== $values['end_at_gmt'] && $values['start_at_gmt'] > $values['end_at_gmt'] ) {
			return self::error( 'spdb_collection_date_range_invalid', 'The end date cannot precede the start date.' );
		}

		if ( 'collection' === $record_type ) {
			if ( '' !== $values['ethical_declaration'] || array() !== $values['target_surfaces'] || '' !== $values['start_at_gmt'] || '' !== $values['end_at_gmt'] ) {
				return self::error( 'spdb_collection_campaign_fields_forbidden', 'Campaign-only ethics, target, and schedule fields are not permitted on a collection.' );
			}
			if ( 'own' === $scope && array() !== $values['contributors'] ) {
				return self::error( 'spdb_collection_contributors_forbidden', 'An own-scope collection cannot delegate contributor authority.' );
			}
		}

		if ( 'campaign' === $record_type ) {
			if ( 'institution' !== $scope || '' === $values['objective'] || '' === $values['ethical_declaration'] || '' === $values['start_at_gmt'] || '' === $values['end_at_gmt'] || array() === $values['target_surfaces'] ) {
			...
			if ( self::contains_prohibited_campaign_pattern( $values['title'] . ' ' . $values['objective'] . ' ' . $values['ethical_declaration'] ) ) {
				return self::error( 'spdb_campaign_ethics_invalid', 'Fear, false urgency, fake scarcity, fabricated metrics, miracle claims, and cure guarantees are prohibited.' );
			}
		}
	...
	public static function validate_knowledge_link( array $input ) {
		...
		$scope = self::enum_value( $input['scope'] ?? '', self::scopes(), 'spdb_knowledge_scope_invalid' );
		if ( is_wp_error( $scope ) ) {
			return $scope;
		}
		$authority = self::mutation_authority( 'knowledge_link', $scope );
		if ( is_wp_error( $authority ) ) {
			return $authority;
		}
		...
		if ( $values['source_provider_key'] === $values['target_provider_key'] && $values['source_object_type'] === $values['target_object_type'] && $values['source_object_id'] === $values['target_object_id'] ) {
	...
	private static function mutation_authority( string $record_type, string $scope ) {
		...
		if ( $user_id < 1 || ! SPDB_Membership_Guard::is_user_approved( $user_id ) ) {
			return self::error( 'spdb_collections_account_forbidden', 'An approved current account is required for collection metadata changes.', 403 );
		}

		if ( 'institution' === $scope || 'campaign' === $record_type ) {
			if ( ! self::current_user_is_founder() || ! SPDB_Capabilities::current_user_can( 'spdb_manage_campaigns' ) ) {
				return self::error( 'spdb_collections_institution_forbidden', 'Institution metadata is Founder-governed and requires the campaign-management capability.', 403 );
			}
			return true;
		}

		if ( ! SPDB_Capabilities::current_user_can( 'spdb_manage_own_content' ) ) {
			return self::error( 'spdb_collections_own_forbidden', 'The current account is not authorized to manage own-scope collection metadata.', 403 );
		}
		return true;
	}
	...
	/**
	 * Defense-in-depth phrase screen only. Human moderation and native medical
	 * policy remain mandatory and cannot be replaced by this bounded check.
	 */
	private static function contains_prohibited_campaign_pattern( string $value ): bool {
		$pattern = '/\b(?:guarantee(?:d|s)?\s+(?:a\s+)?cure|cure\s+guarantee(?:d|s)?|miracle\s+cure|instant\s+cure|100%\s+(?:success|effective)|no\s+risk|limited\s+(?:seats|time|stock)\s+only|act\s+now|last\s+chance|do\s+not\s+miss|fear\s+of\s+death|everyone\s+is\s+buying|doctor(?:s)?\s+do\s+not\s+want\s+you\s+to\s+know)\b/i';
	...
	private static function plain_text( $raw, int $maximum, bool $allow_empty, string $code ) {
		...
		if ( ( ! $allow_empty && '' === $value ) || self::text_length( $value ) > $maximum || wp_strip_all_tags( $value ) !== $value ) {
			return self::error( $code, 'A metadata text field is invalid.' );
		}
		if ( preg_match( '/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/', $value ) || preg_match( '/(?:https?:\/\/|[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}|\b(?:\+?92|0)?3\d{9}\b|\b\d{5}-\d{7}-\d\b)/iu', $value ) ) {
	...
	/**
	 * Shape validation only. The runtime service must recheck every contributor
	 * against current File 00 status and the required capability.
	 *
	 * @return int[]|WP_Error
	 */
	private static function positive_integer_list( $raw, int $maximum, string $code ) {
		...
		foreach ( $raw as $item ) {
			...
			if ( $value < 1 || $value > 2147483647 || in_array( $value, $result, true ) ) {
