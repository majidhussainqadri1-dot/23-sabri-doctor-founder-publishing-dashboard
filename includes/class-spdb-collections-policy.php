<?php
/** Phase 23F policy boundary for cross-module metadata. */
...
final class SPDB_Collections_Policy {
	...
	public static function statuses(): array { return array( 'draft', 'active', 'paused', 'completed', 'archived' ); }
	...
	public static function validate_collection( array $input ) {
		...
		if ( strlen( $values['audit_reason'] ) < 10 ) {
		...
		if ( 'institution' === $values['scope'] && ! $is_founder ) {
			return self::error( 'spdb_collection_institution_forbidden', 'Institution metadata is Founder-governed.' );
		}
		if ( 'campaign' === $values['record_type'] ) {
			if ( ! $is_founder ) {
				return self::error( 'spdb_campaign_founder_required', 'Only the Founder may create an institution campaign.' );
			}
			if ( 'institution' !== $values['scope'] || '' === $values['objective'] || '' === $values['ethical_declaration'] || '' === $values['start_at_gmt'] || '' === $values['end_at_gmt'] || array() === $values['target_surfaces'] ) {
			...
			if ( self::contains_prohibited_campaign_pattern( $values['objective'] . ' ' . $values['ethical_declaration'] ) ) {
	...
	public static function validate_knowledge_link( array $input ) {
		$allowed = array( 'source_provider_key', 'source_object_type', 'source_object_id', 'target_provider_key', 'target_object_type', 'target_object_id', 'relation_type', 'idempotency_key', 'audit_reason' );
		if ( array_diff( array_keys( $input ), $allowed ) ) {
	...
	private static function plain_text( $raw, int $maximum, bool $allow_empty, string $code ) {
		...
		if ( ( ! $allow_empty && '' === $value ) || strlen( $value ) > $maximum || wp_strip_all_tags( $value ) !== $value ) { return self::error( $code, 'A metadata text field is invalid.' ); }
		if ( preg_match( '/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/', $value ) || preg_match( '/(?:https?:\/\/|[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}|\b(?:\+?92|0)?3\d{9}\b|\b\d{5}-\d{7}-\d\b)/i', $value ) ) { return self::error( $code, 'A metadata text field contains prohibited sensitive data.' ); }
		return $value;
	}
	/** @return string|WP_Error */
	...
	private static function positive_integer_list( $raw, int $maximum, string $code ) {
		...
		foreach ( $raw as $item ) {
			...
			if ( $value < 1 || in_array( $value, $result, true ) ) { return self::error( $code, 'The contributor list contains an invalid or duplicate user identifier.' ); }
			$result[] = $value;
		}
		return $result;
	}
	...
	private static function canonical_list( $raw, array $allowed, int $maximum, string $code ) {
		...
		foreach ( $raw as $item ) {
			...
			if ( ! in_array( $value, $allowed, true ) || in_array( $value, $result, true ) ) { return self::error( $code, 'A canonical metadata list contains an invalid or duplicate value.' ); }
			$result[] = $value;
		}
		return $result;
	}
	private static function is_list( array $value ): bool {
		$expected = 0;
		foreach ( $value as $key => $unused ) { if ( $key !== $expected ) { return false; } ++$expected; }
		return true;
	}
