...
final class SPDB_REST_Rate_Limiter {
	...
	public static function verify() {
		global $wpdb;
		if ( self::SCHEMA_VERSION !== (string) get_option( self::VERSION_OPTION, '' ) || ! is_object( $wpdb ) || empty( $wpdb->prefix ) ) {
			return self::error( 'spdb_rate_limit_schema_unverified', __( 'The dashboard rate-limit schema is not verified.', 'sabri-publishing-dashboard' ), 503 );
		}
		...
		return 'innodb' === strtolower( $engine )
			? true
			: self::error( 'spdb_rate_limit_table_not_transactional', __( 'The dashboard rate-limit store is not transaction-safe.', 'sabri-publishing-dashboard' ), 503 );
	}

	/** Explicit local-repair entry point; caller remains responsible for authorization and audit. @return true|WP_Error */
	public static function repair_schema() {
		return self::install();
	}

	/** @param mixed $result Existing pre-dispatch response. @return mixed */
	public static function enforce( $result, WP_REST_Server $server, WP_REST_Request $request ) {
		unset( $server );
		if ( null !== $result ) { return $result; }
		$route = (string) $request->get_route();
		if ( ! self::is_file23_route( $route ) ) { return null; }
		...
		if ( empty( $claim['allowed'] ) ) {
			return new WP_Error(
				...
				array( 'status' => 429, 'retry_after' => max( 1, (int) $claim['retry_after'] ), 'policy' => (string) $policy['key'] )
			);
		}
		return null;
	}
	...
	public static function policy_for( string $route, string $method ): ?array {
		...
		if ( ! $write ) { return array( 'key' => 'read', 'limit' => 240, 'window' => MINUTE_IN_SECONDS ); }
		if ( 1 === preg_match( '#^/spdb/v1/ai-assistance$#', $route ) ) { return array( 'key' => 'ai', 'limit' => 20, 'window' => MINUTE_IN_SECONDS ); }
		if ( 1 === preg_match( '#^/spdb/v1/exports$#', $route ) ) { return array( 'key' => 'export', 'limit' => 10, 'window' => MINUTE_IN_SECONDS ); }
		if ( 1 === preg_match( '#^/spdb/v1/(?:settings|activation|system-check/repair|provider-acceptance/[a-z0-9][a-z0-9_-]{1,63})$#', $route ) ) { return array( 'key' => 'privileged', 'limit' => 20, 'window' => MINUTE_IN_SECONDS ); }
		if ( 1 === preg_match( '#^/spdb/v1/(?:review|calendar)/#', $route ) ) { return array( 'key' => 'native-write', 'limit' => 30, 'window' => MINUTE_IN_SECONDS ); }
	...
	private static function claim( string $bucket_hash, int $actor_user_id, string $policy_key, int $limit, int $window ) {
		...
		$now = time(); $started = gmdate( 'Y-m-d H:i:s', $now ); $reset = gmdate( 'Y-m-d H:i:s', $now + $window );
		$sql = $wpdb->prepare(
			...
			$bucket_hash, $actor_user_id, $policy_key, $started, $reset, $started
		);
		...
		$row = $wpdb->get_row( $wpdb->prepare( "SELECT request_count, reset_at_gmt FROM {$table} WHERE bucket_hash = %s LIMIT 1", $bucket_hash ), defined( 'ARRAY_A' ) ? ARRAY_A : 'ARRAY_A' );
		...
		$count = max( 0, (int) $row['request_count'] ); $reset_epoch = strtotime( (string) $row['reset_at_gmt'] . ' UTC' );
		...
		return array( 'allowed' => $count <= $limit, 'retry_after' => max( 1, $reset_epoch - time() ), 'count' => $count );
	}
	...
	private static function subject_key( int $actor_user_id ): string {
		if ( $actor_user_id > 0 ) { return 'user:' . $actor_user_id; }
		$remote = isset( $_SERVER['REMOTE_ADDR'] ) && is_scalar( $_SERVER['REMOTE_ADDR'] ) ? trim( (string) $_SERVER['REMOTE_ADDR'] ) : '';
		if ( '' === $remote || false === filter_var( $remote, FILTER_VALIDATE_IP ) ) { $remote = 'unknown'; }
	...
	public static function cleanup(): void {
		...
		if ( '' === $table || ! is_object( $wpdb ) ) { return; }
		for ( $batch = 0; $batch < 10; ++$batch ) {
			...
			if ( ! is_int( $deleted ) || $deleted < 1000 ) { break; }
		}
	}
