...
final class SPDB_Operational_Projection_Validator {
	...
	private const DOMAINS = array(
		'sources','media','interactions','gaps','revisions','notifications','appointments','messages','reviews','followers','downloads','support','learning','ai_teacher',
	);
	private const PRIVACY = array( 'public', 'restricted', 'private', 'clinical_sensitive' );
	private const STATUS_ONLY_DOMAINS = array( 'appointments', 'messages' );
	...
	public static function projection_page( $raw, string $provider_key, string $domain ) {
		...
		foreach ( $raw['items'] as $row ) {
			...
			if ( is_wp_error( $validated ) ) { return self::error( 'spdb_projection_invalid_page_item', 'The provider projection contained an invalid item and was rejected.' ); }
			$items[] = $validated;
		}
		if ( array_key_exists( 'total', $raw ) && ( ! is_int( $raw['total'] ) || $raw['total'] < count( $items ) || $raw['total'] > 1000000 ) ) { return self::error( 'spdb_projection_total_invalid', 'The provider returned an invalid projection total.' ); }
		if ( array_key_exists( 'has_more', $raw ) && ! is_bool( $raw['has_more'] ) ) { return self::error( 'spdb_projection_pagination_invalid', 'The provider returned invalid projection pagination.' ); }
		$total = $raw['total'] ?? count( $items );
		$has_more = true === ( $raw['has_more'] ?? false );
		if ( $has_more && $total <= count( $items ) ) { return self::error( 'spdb_projection_pagination_invalid', 'The provider returned inconsistent projection pagination.' ); }
		$generated_at = self::timestamp( $raw['generated_at'] ?? gmdate( 'c' ) );
		if ( null === $generated_at ) { return self::error( 'spdb_projection_timestamp_invalid', 'The provider returned an invalid projection timestamp.' ); }
		return array( 'items' => $items, 'total' => $total, 'has_more' => $has_more, 'generated_at' => $generated_at );
	}
	...
	public static function projection_item( $row, string $provider_key, string $domain ) {
		...
		if ( self::contains_direct_identifier( $title ) || self::contains_direct_identifier( $summary ) ) {
			return self::error( 'spdb_projection_sensitive_text', 'The provider projection contains prohibited personal or credential data.' );
		}
		...
		$destination = '';
		if ( isset( $row['destination_url'] ) && is_scalar( $row['destination_url'] ) ) { $destination = self::safe_destination( (string) $row['destination_url'] ); }
		$metadata = array();
		if ( isset( $row['metadata'] ) && is_array( $row['metadata'] ) ) {
			foreach ( array_slice( $row['metadata'], 0, 20, true ) as $key => $value ) {
				$raw_key = trim( (string) $key ); $key = sanitize_key( $raw_key );
				if ( '' === $key || $key !== $raw_key || ! is_scalar( $value ) || self::sensitive_key( $key ) ) { continue; }
				$text = trim( wp_strip_all_tags( (string) $value ) );
				if ( self::contains_direct_identifier( $text ) ) { continue; }
				if ( self::bounded_text( $text, 240, true ) ) { $metadata[ $key ] = $text; }
			}
		}

		/*
		 * Messages and appointments are status-only in File 23: their private body,
		 * correspondent/patient identity, schedule details and native destinations
		 * remain with their canonical modules. `clinical_sensitive` projections are
		 * likewise reduced to a non-identifying status envelope.
		 */
		if ( in_array( $domain, self::STATUS_ONLY_DOMAINS, true ) || 'clinical_sensitive' === $privacy ) {
			$title       = 'Native ' . ( 'appointments' === $domain ? 'appointment' : ( 'messages' === $domain ? 'message' : 'restricted item' ) );
			$summary     = '';
			$destination = '';
			$metadata    = array();
		}
		return array(
			'provider_key' => $provider_key, 'domain' => $domain, 'object_type' => $object_type, 'object_id' => $object_id,
			'native_version' => $native_version, 'title' => $title, 'summary' => self::text_substr( $summary, 0, 500 ), 'status' => $status,
			'severity' => $severity, 'privacy_class' => $privacy, 'updated_at' => $updated_at, 'destination_url' => $destination, 'metadata' => $metadata,
		);
	}
	...
	public static function metrics( $raw, string $provider_key, int $configured_threshold ) {
		...
		foreach ( $raw['metrics'] as $metric ) {
			...
			$key_raw = trim( (string) ( $metric['metric_key'] ?? '' ) ); $key = sanitize_key( $key_raw );
			$label = trim( wp_strip_all_tags( (string) ( $metric['label'] ?? '' ) ) );
			$definition = trim( wp_strip_all_tags( (string) ( $metric['definition'] ?? '' ) ) );
			$interval_raw = trim( (string) ( $metric['interval'] ?? '' ) ); $interval = sanitize_key( $interval_raw );
			...
			$unit_raw = trim( (string) ( $metric['unit'] ?? 'count' ) ); $unit = sanitize_key( $unit_raw );
			...
			$value = null; $suppressed = $cohort < $threshold;
			...
			$items[] = array( 'provider_key' => $provider_key, 'metric_key' => $key, 'label' => $label, 'definition' => $definition, 'value' => $value, 'unit' => $unit, 'interval' => $interval, 'cohort_count' => $cohort, 'privacy_threshold' => $threshold, 'suppressed' => $suppressed, 'generated_at' => $generated_at );
		}
		$generated_at = self::timestamp( $raw['generated_at'] ?? gmdate( 'c' ) );
		return null === $generated_at ? self::error( 'spdb_metrics_timestamp_invalid', 'The provider returned an invalid analytics timestamp.' ) : array( 'metrics' => $items, 'generated_at' => $generated_at );
	}
	...
	private static function safe_destination( string $url ): string { $normalized = SPDB_Safe_Destination::normalize( $url ); return is_wp_error( $normalized ) ? '' : $normalized; }
	private static function canonical_key( string $value ): bool { return 1 === preg_match( '/^[a-z0-9][a-z0-9_-]{0,63}$/', $value ); }
	private static function bounded_text( string $value, int $limit, bool $allow_empty = false ): bool { return ( $allow_empty || '' !== $value ) && self::valid_utf8( $value ) && self::text_length( $value ) <= $limit && 1 !== preg_match( '/[\x00-\x1F\x7F]/u', $value ); }
	private static function valid_utf8( string $value ): bool { return 1 === preg_match( '//u', $value ); }
	private static function text_length( string $value ): int { return function_exists( 'mb_strlen' ) ? mb_strlen( $value, 'UTF-8' ) : strlen( $value ); }
	private static function text_substr( string $value, int $start, int $length ): string { return function_exists( 'mb_substr' ) ? mb_substr( $value, $start, $length, 'UTF-8' ) : substr( $value, $start, $length ); }
	private static function sensitive_key( string $key ): bool { return 1 === preg_match( '/(?:patient|email|phone|address|message_body|message_text|conversation|recipient|sender|ip|token|secret|password|consent_document|appointment|clinical|diagnosis|prescription|payment|billing|national_id|passport|guardian)/', $key ); }
	private static function contains_direct_identifier( string $value ): bool {
		if ( '' === $value ) { return false; }
		return 1 === preg_match( '/(?:[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}|\b\+?\d[\d ()-]{7,}\d\b|\b(?:cnic|passport|national[ _-]?id|guardian[ _-]?(?:phone|email)|patient[ _-]?(?:name|id)|message[ _-]?body|auth(?:orization)?[ _-]?token|api[ _-]?key|password)\b)/i', $value );
	}
