...
final class SPDB_Local_Repair {
	...
	public function system_check(): array {
		...
		$checks = array(
			...
			'collections_schema' => array( 'ok' => true === $collection_schema, 'code' => true === $collection_schema ? 'ready' : 'schema_unavailable' ),
			'operations_schema' => array( 'ok' => true === $operations_schema, 'code' => true === $operations_schema ? 'ready' : 'schema_unavailable' ),
		...
		$healthy = true;
		foreach ( $checks as $check ) { $healthy = $healthy && ! empty( $check['ok'] ); }
	...
	public function repair( string $action, string $reason ) {
		...
		switch ( $action ) {
			...
			case 'install_operations_schema': $result = SPDB_Operations_Schema::install(); break;
			...
			case 'reschedule_jobs':
				if ( ! wp_next_scheduled( SPDB_Background_Jobs::HOOK ) ) { wp_schedule_event( time() + MINUTE_IN_SECONDS, 'spdb_five_minutes', SPDB_Background_Jobs::HOOK ); }
