...
build_package() {
  ...
  find "$root" -exec touch -h -t 202608040000.00 {} +
  (
    cd "$root"
...
unzip -p "$package_name" sabri-publishing-dashboard/sabri-publishing-dashboard.php | grep -Eq '^ \* Version:[[:space:]]+1\.0\.0$'
unzip -p "$package_name" sabri-publishing-dashboard/readme.txt | grep -Fq 'Stable tag: 1.0.0'
