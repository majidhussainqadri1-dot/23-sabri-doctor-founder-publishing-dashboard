...
build_package() {
  ...
  find "$root" -exec touch -h -t 202608041335.00 {} +
  (
    cd "$root"
...
unzip -p "$package_name" sabri-publishing-dashboard/sabri-publishing-dashboard.php | grep -Eq '^ \* Version:[[:space:]]+1\.2\.0$'
unzip -p "$package_name" sabri-publishing-dashboard/readme.txt | grep -Fq 'Stable tag: 1.2.0'
...
unzip -p "$source_package" "${source_root}/sabri-publishing-dashboard.php" | grep -Eq '^ \* Version:[[:space:]]+1\.2\.0$'
