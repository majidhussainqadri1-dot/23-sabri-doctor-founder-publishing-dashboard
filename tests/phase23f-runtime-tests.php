...
$GLOBALS['spdb_test_member_status'] = 'submitted'; $GLOBALS['spdb_test_member_statuses'][7] = 'submitted'; $pending = $service->list_collections( array( 'scope' => 'own' ) ); spdb_runtime_assert( 'spdb_collections_read_forbidden' === spdb_runtime_code( $pending ), 'Pending accounts must not read collection metadata.' );
$GLOBALS['spdb_test_member_status'] = 'approved'; $GLOBALS['spdb_test_member_statuses'][7] = 'approved'; $repository->collections['collection_123e4567e89b12d3a456000000000099'] = spdb_runtime_collection_row();
...
$repository->inject_unknown = true; $unknown = $service->list_collections( array( 'scope' => 'own' ) ); spdb_runtime_assert( 'spdb_collections_repository_response_invalid' === spdb_runtime_code( $unknown ), 'Unknown repository projection fields must fail closed before returning data.' ); $repository->inject_unknown = false;
$repository->inject_malformed = true; $malformed = $service->list_collections( array( 'scope' => 'own' ) ); spdb_runtime_assert( 'spdb_collections_repository_response_invalid' === spdb_runtime_code( $malformed ), 'Malformed numeric owner and version fields must fail closed.' ); $repository->inject_malformed = false;

/* Mutation success paths require a current canonical publishing identity, not generic approval plus a stale role capability. */
$GLOBALS['spdb_test_founder'] = true;
$GLOBALS['spdb_test_environment'] = 'staging';
$created = $service->create_collection( spdb_runtime_collection_input() ); spdb_runtime_assert( is_array( $created ) && 7 === $created['owner_user_id'], 'A reviewed staging collection create by the current Founder must not require a native resolver.' );
...
$resolver = new SPDB_Test_Native_Resolver(); $service = new SPDB_Collections_Service( $repository, $resolver ); $link = $service->create_knowledge_link( spdb_runtime_link_input() ); spdb_runtime_assert( is_array( $link ) && 'v1' === $link['source_native_version'], 'A valid Founder knowledge link must persist observed native versions.' );
