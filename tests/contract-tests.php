...
function spdb_test_assert( bool $condition, string $message ): void {
	global $tests, $failed;
	++$tests;
	if ( ! $condition ) {
		++$failed;
...
function spdb_test_error_code( $value ): string {
...
$incompatible = ( new SPDB_Adapter_Registry() )->register(
	new SPDB_Test_Adapter(
		array(
			'minimum_contract' => '3.0.0',
			'maximum_contract' => '3.9.9',
		)
	)
);
...
$self_accepted = ( new SPDB_Adapter_Registry() )->register(
	new SPDB_Test_Adapter( array( 'capability_state' => SPDB_Adapter_Registry::ACCEPTANCE_PRODUCTION_ACCEPTED ) )
);
...
$missing_version = $broker->execute(
	'provider_one',
	'submit_item',
	'publication',
	'42',
	array( 'idempotency_key' => '1234567890abcdef' )
);
...
if ( $failed > 0 ) {
	fwrite( STDERR, "{$failed} of {$tests} contract tests failed.\n" );
	exit( 1 );
}
