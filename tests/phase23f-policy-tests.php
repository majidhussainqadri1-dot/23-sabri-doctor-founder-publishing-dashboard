...
function spdb_23f_code( $value ): string { return $value instanceof WP_Error ? $value->get_error_code() : ''; }
function spdb_23f_collection( array $overrides = array() ): array {
	return array_merge(
		array(
			'record_type' => 'collection', 'scope' => 'own', 'title' => 'Classical Homeopathy Study Set',
			'objective' => '', 'ethical_declaration' => '', 'contributors' => array(), 'target_surfaces' => array(),
			'status' => 'draft', 'start_at_gmt' => '', 'end_at_gmt' => '',
...
function spdb_23f_link( array $overrides = array() ): array {
	return array_merge(
		array(
			'scope' => 'own', 'source_provider_key' => 'file21', 'source_object_type' => 'publication', 'source_object_id' => 'post-101',
			'target_provider_key' => 'file06', 'target_object_type' => 'remedy', 'target_object_id' => 'remedy-22',
			'relation_type' => 'encyclopedia', 'idempotency_key' => 'knowledge-link-0001',
...
$generic_approved = SPDB_Collections_Policy::validate_collection( spdb_23f_collection() );
spdb_23f_assert( 'spdb_collections_own_forbidden' === spdb_23f_code( $generic_approved ), 'Generic approved membership must not obtain publishing collection authority from a stale WordPress capability.' );
$GLOBALS['spdb_test_founder'] = true;
$own = SPDB_Collections_Policy::validate_collection( spdb_23f_collection() );
spdb_23f_assert( is_array( $own ) && 'own' === $own['scope'], 'A current Founder with explicit capability may validate an own-scope collection.' );
$GLOBALS['spdb_test_founder'] = false;
...
spdb_23f_assert( 'spdb_collections_own_forbidden' === spdb_23f_code( $without_capability ), 'Own-scope metadata requires current publishing identity and management capability.' );
$GLOBALS['spdb_test_capabilities']['spdb_manage_own_content'] = true;
...
spdb_23f_assert( 'spdb_collections_own_forbidden' === spdb_23f_code( $contributors_forbidden ), 'A generic approved non-doctor is denied before contributor delegation semantics are evaluated.' );
$campaign_fields = SPDB_Collections_Policy::validate_collection( spdb_23f_collection( array( 'target_surfaces' => array( 'news' ) ) ) );
spdb_23f_assert( 'spdb_collections_own_forbidden' === spdb_23f_code( $campaign_fields ), 'A generic approved non-doctor is denied before campaign-only field validation.' );
$bad_date = SPDB_Collections_Policy::validate_collection( spdb_23f_collection( array( 'start_at_gmt' => '2026-02-31T00:00:00Z' ) ) );
spdb_23f_assert( 'spdb_collections_own_forbidden' === spdb_23f_code( $bad_date ), 'A generic approved non-doctor is denied before timestamp validation.' );
...
$campaign = SPDB_Collections_Policy::validate_collection(
	spdb_23f_collection(
		array(
			'record_type' => 'campaign', 'scope' => 'institution', 'title' => 'Source Citation Improvement',
			...
			'contributors' => array( 7, 9 ), 'target_surfaces' => array( 'news', 'learning', 'encyclopedia' ),
			'start_at_gmt' => '2026-08-01T00:00:00Z', 'end_at_gmt' => '2026-08-31T23:59:59Z',
			'idempotency_key' => 'campaign-create-0001', 'audit_reason' => 'Create a governed source-quality campaign for approved content.',
		)
	)
);
...
spdb_23f_assert( is_array( $link ) && 'own' === $link['scope'], 'A current Founder may validate an own-scope knowledge link.' );
$GLOBALS['spdb_test_founder'] = false;
$GLOBALS['spdb_test_capabilities']['spdb_manage_campaigns'] = false;
...
spdb_23f_assert( 'spdb_collections_own_forbidden' === spdb_23f_code( $self_link ), 'A generic approved non-doctor is denied before self-link validation.' );
$bad_relation = SPDB_Collections_Policy::validate_knowledge_link( spdb_23f_link( array( 'relation_type' => 'unrestricted_relation' ) ) );
spdb_23f_assert( 'spdb_collections_own_forbidden' === spdb_23f_code( $bad_relation ), 'A generic approved non-doctor is denied before relation validation.' );
...
if ( $failed > 0 ) { fwrite( STDERR, "{$failed} of {$tests} corrective Phase 23F tests failed.\n" ); exit( 1 ); }
