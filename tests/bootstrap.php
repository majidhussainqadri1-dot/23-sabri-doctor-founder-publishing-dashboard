...
if ( ! class_exists( 'SPDB_Test_Role' ) ) {
	class SPDB_Test_Role {
		...
		public function add_cap( string $capability, bool $grant = true ): void { $this->capabilities[ $capability ] = $grant; }
	}
}
...
function add_query_arg( $key, $value = null, $url = null ): string {
	if ( is_array( $key ) ) {
		$base = (string) $value;
		foreach ( $key as $query_key => $query_value ) { $base = add_query_arg( (string) $query_key, (string) $query_value, $base ); }
		return $base;
	}
	$url = (string) $url;
...
function update_user_meta( int $user_id, string $key, $value, $previous_value = null ): bool {
	...
	if ( ! empty( $GLOBALS['spdb_test_force_meta_conflict'] ) ) { $GLOBALS['spdb_test_user_meta'][ $user_id ][ $key ] = array( 'concurrent_change' => true ); $GLOBALS['spdb_test_force_meta_conflict'] = false; return false; }
	if ( 4 === func_num_args() && $current !== $previous_value ) { return false; }
