<?php
/** Executable WordPress repository tests for the corrected Phase 23F slice. */
require_once __DIR__ . '/bootstrap.php';

final class SPDB_Test_WPDB {
	public string $prefix = 'wp_';
	public array $rows = array();
	public bool $missing_index = false;
	public int $schema_inspections = 0;
	...
	public function get_var( string $query ) {
		if ( preg_match( "/SHOW TABLES LIKE '([^']+)'/", $query, $match ) ) { ++$this->schema_inspections; return array_key_exists( str_replace( '\\_', '_', $match[1] ), $this->rows ) ? str_replace( '\\_', '_', $match[1] ) : null; }
		if ( false !== stripos( $query, 'SELECT COUNT(*)' ) ) { return (string) count( $this->filter_select( $query ) ); }
		if ( false !== stripos( $query, 'SELECT link_id' ) ) { $rows = $this->filter_select( $query ); return $rows[0]['link_id'] ?? null; }
		return null;
	}
	public function get_results( string $query, $output = null ) {
		if ( preg_match( '/SHOW COLUMNS FROM `([^`]+)`/', $query, $match ) ) { ++$this->schema_inspections; $requirements = SPDB_Collections_Schema::requirements(); return array_map( static fn( $field ) => array( 'Field' => $field ), $requirements[ $match[1] ]['columns'] ?? array() ); }
		if ( preg_match( '/SHOW INDEX FROM `([^`]+)`/', $query, $match ) ) { ++$this->schema_inspections; $requirements = SPDB_Collections_Schema::requirements(); $indexes = $requirements[ $match[1] ]['indexes'] ?? array(); if ( $this->missing_index ) { array_pop( $indexes ); } return array_map( static fn( $name ) => array( 'Key_name' => $name ), $indexes ); }
	...
	public function insert( string $table, array $data, $format = null ) {
		...
		if ( 'wp_spdb_knowledge_links' === $table ) {
			foreach ( $this->rows[ $table ] as $row ) {
				if ( (int) $row['owner_user_id'] === (int) $data['owner_user_id'] && $row['scope'] === $data['scope'] && $row['relation_hash'] === $data['relation_hash'] ) { return false; }
			}
		}
	...
	private function filter_select( string $query ): array {
		...
		$filters = array(
			'collection_id' => "/collection_id = '([^']+)'/", 'link_id' => "/link_id = '([^']+)'/", 'item_id' => "/item_id = '([^']+)'/", 'scope' => "/scope = '([^']+)'/", 'record_type' => "/record_type = '([^']+)'/", 'status' => "/status = '([^']+)'/", 'idempotency_hash' => "/idempotency_hash = '([^']+)'/", 'relation_hash' => "/relation_hash = '([^']+)'/",
		);
		...
		$offset = preg_match( '/OFFSET (\d+)/', $query, $match ) ? (int) $match[1] : 0;
		$limit = preg_match( '/LIMIT (\d+)/', $query, $match ) ? (int) $match[1] : count( $rows );
...
$unknown_record = $record; $unknown_record['native_destination'] = 'https://example.test/private'; spdb_repository_assert( 'spdb_collection_record_invalid' === spdb_repository_code( $repository->create_collection( $unknown_record ) ), 'Unknown persistence fields must fail closed.' );
$malformed_query = $repository->list_collections( array( 'scope' => 'own', 'owner_user_id' => '7abc', 'record_type' => 'collection', 'status' => 'draft', 'page' => 1, 'per_page' => 20 ) ); spdb_repository_assert( 'spdb_repository_query_invalid' === spdb_repository_code( $malformed_query ), 'Malformed direct repository owner IDs must not be cast into valid IDs.' );
$overflow_query = $repository->list_collections( array( 'scope' => 'own', 'owner_user_id' => str_repeat( '9', 80 ), 'record_type' => 'collection', 'status' => 'draft', 'page' => 1, 'per_page' => 20 ) ); spdb_repository_assert( 'spdb_repository_query_invalid' === spdb_repository_code( $overflow_query ), 'Overflowing decimal owner IDs must not saturate into valid integers.' );
$impossible_query = $repository->list_collections( array( 'scope' => 'own', 'owner_user_id' => 7, 'record_type' => 'collection', 'status' => 'paused', 'page' => 1, 'per_page' => 20 ) ); spdb_repository_assert( 'spdb_repository_query_invalid' === spdb_repository_code( $impossible_query ), 'Direct repository filters must bind status to record type.' );
$invalid_utf8 = $record; $invalid_utf8['title'] = "\xC3\x28"; $invalid_utf8['idempotency_hash'] = str_repeat( '3', 64 ); $invalid_utf8['request_hash'] = str_repeat( '4', 64 ); spdb_repository_assert( 'spdb_collection_record_invalid' === spdb_repository_code( $repository->create_collection( $invalid_utf8 ) ), 'Invalid UTF-8 metadata must fail closed at the persistence boundary.' );
$list = $repository->list_collections( array( 'scope' => 'own', 'owner_user_id' => 7, 'record_type' => 'collection', 'status' => 'draft', 'page' => 1, 'per_page' => 20 ) ); spdb_repository_assert( is_array( $list ) && 1 === $list['total'] && 1 === count( $list['items'] ) && false === $list['has_more'], 'The concrete repository must return a bounded truthful collection envelope.' );
$wpdb->rows['wp_spdb_collections'][0]['archived_at_gmt'] = array( 'malformed' );
$malformed_timestamp = $repository->get_collection( $created['collection_id'] );
spdb_repository_assert( is_array( $malformed_timestamp ) && '__invalid_timestamp__' === $malformed_timestamp['archived_at_gmt'], 'Malformed optional database timestamps must remain visibly invalid instead of becoming empty.' );
$wpdb->rows['wp_spdb_collections'][0]['archived_at_gmt'] = null;
...
$duplicate_relation = $link_record; $duplicate_relation['idempotency_hash'] = str_repeat( '1', 64 ); $duplicate_relation['request_hash'] = str_repeat( '2', 64 ); spdb_repository_assert( 'spdb_knowledge_relation_conflict' === spdb_repository_code( $repository->create_knowledge_link( $duplicate_relation ) ), 'A duplicate canonical relation with a new idempotency key must return a deterministic conflict.' );
