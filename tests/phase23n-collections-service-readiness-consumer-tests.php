<?php
/** Executable tests for corrected Phase 23N service readiness consumer. */
...
final class SPDB_23N_Repository implements SPDB_Collections_Repository {
	...
	public function __construct() { $this->health = spdb_23n_health(); }
	public function health_check(): array {
		++$this->health_calls;
		if ( $this->throw ) { throw new RuntimeException( 'private Phase 23N repository exception' ); }
		return $this->health;
	}
...
function spdb_23n_code( $value ): string { return $value instanceof WP_Error ? $value->get_error_code() : ''; }
...
/** @return array<string,mixed> */
function spdb_23n_snapshot( array $changes = array() ): array {
	return array_merge(
		array(
			'inputs_valid' => true,
...
spdb_23n_assert( true === spdb_23n_private( $consumer, 'resolver_readiness_available' ), 'Formal resolver readiness capability must be stored separately from presence.' );
...
try { $consumer->__wakeup(); } catch ( LogicException $exception ) { $wakeup_blocked = true; }
spdb_23n_assert( $wakeup_blocked, 'The consumer must reject wakeup reconstruction.' );

$validator = new ReflectionMethod( SPDB_Collections_Service_Readiness_Consumer::class, 'valid_snapshot' );
$validator->setAccessible( true );
$canonical_snapshot = spdb_23n_snapshot();
spdb_23n_assert( spdb_23n_exact_keys( $canonical_snapshot, $snapshot_keys ), 'The canonical raw snapshot fixture must contain the exact seventeen fields.' );
spdb_23n_assert( true === $validator->invoke( $consumer, $canonical_snapshot ), 'The exact canonical knowledge-ready raw snapshot must validate.' );
$extra_snapshot = $canonical_snapshot; $extra_snapshot['private_detail'] = 'forbidden';
spdb_23n_assert( false === $validator->invoke( $consumer, $extra_snapshot ), 'An extra raw snapshot field must fail closed.' );
$missing_snapshot = $canonical_snapshot; unset( $missing_snapshot['resolver_code'] );
spdb_23n_assert( false === $validator->invoke( $consumer, $missing_snapshot ), 'A missing raw snapshot field must fail closed.' );
$unknown_repository_code = $canonical_snapshot; $unknown_repository_code['repository_code'] = 'database_table_missing';
spdb_23n_assert( false === $validator->invoke( $consumer, $unknown_repository_code ), 'An unknown canonical repository code must fail closed.' );
$unknown_integration_code = $canonical_snapshot; $unknown_integration_code['integration_code'] = 'custom_ready';
spdb_23n_assert( false === $validator->invoke( $consumer, $unknown_integration_code ), 'An unknown canonical integration code must fail closed.' );
$wrong_scalar = $canonical_snapshot; $wrong_scalar['write_configured'] = 1;
spdb_23n_assert( false === $validator->invoke( $consumer, $wrong_scalar ), 'A weakly typed raw authority scalar must fail closed.' );
...
$repository->health_calls = 0; $resolver->reset();
spdb_23n_assert( true === $consumer->require_read_ready(), 'Ready repository must satisfy consumer-owned read readiness.' );
spdb_23n_assert( 1 === $repository->health_calls && 0 === $resolver->ready_calls, 'Read readiness must remain resolver-independent and use one repository acquisition.' );

$repository->health_calls = 0; $resolver->reset();
spdb_23n_assert( 'spdb_collections_readiness_input_invalid' === spdb_23n_code( $consumer->require_write_ready( 'yes', true ) ), 'Weakly coercible resolver requirement must fail closed.' );
spdb_23n_assert( 'spdb_collections_readiness_input_invalid' === spdb_23n_code( $consumer->require_write_ready( true, 'yes' ) ), 'Weakly coercible write authority must fail closed in the consumer-owned requirement path.' );
spdb_23n_assert( 0 === $repository->health_calls && 0 === $resolver->ready_calls, 'Invalid requirements must short-circuit every readiness source.' );
...
$resolver->ready = false; $repository->health_calls = 0; $resolver->reset();
...
spdb_23n_assert( 'spdb_native_reference_resolver_not_ready' === spdb_23n_code( $consumer->require_write_ready( true, true ) ), 'Consumer-owned requirements must preserve the stable unready-resolver denial.' );
$resolver->ready = true;
...
spdb_23n_assert( false === spdb_23n_private( $missing_readiness, 'resolver_readiness_available' ), 'Resolver presence must not be confused with formal readiness capability.' );
...
$health_validator = new ReflectionMethod( SPDB_Collections_Service_Readiness_Consumer::class, 'valid_health_projection' );
$health_validator->setAccessible( true );
$forged = $enabled;
$forged['knowledge_write_ready'] = true;
$forged['collection_write_ready'] = true;
$forged['any_write_ready'] = true;
$forged['write_enabled'] = true;
spdb_23n_assert( false === $health_validator->invoke( $missing_readiness, $forged ), 'Knowledge readiness must require formal readiness capability, not resolver presence alone.' );

$repository->throw = true; $repository->health_calls = 0; $resolver->reset();
...
$repository->health = spdb_23n_health( array( 'private_detail' => 'forbidden' ) ); $resolver->reset();
...
$repository->health = spdb_23n_health( array( 'healthy' => false, 'database_ready' => false, 'code' => 'database_unavailable', 'cached_for_request' => false ) ); $resolver->reset();
...
$repository_two = new SPDB_23N_Repository(); $resolver_two = new SPDB_23N_Resolver();
$consumer_two = SPDB_Collections_Service_Readiness_Consumer::create( $repository_two, $resolver_two );
$repository->health_calls = 0; $resolver->reset(); $repository_two->health_calls = 0; $resolver_two->reset();
...
if ( $failed > 0 ) { fwrite( STDERR, "{$failed} of {$tests} corrected Phase 23N readiness consumer tests failed.\n" ); exit( 1 ); }
echo "All {$tests} corrected Phase 23N service readiness consumer tests passed.\n";
