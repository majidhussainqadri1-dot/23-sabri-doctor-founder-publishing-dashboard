...
$assert( str_contains( $baseline, 'awk -F"\'"' ) && ! str_contains( $baseline, 'Version:     1.2.5' ), 'Baseline workflow must use corrected dynamic version extraction and not freeze Version 1.2.5.' );
...
$assert( str_contains( $audit, 'File 00 production blocker remains visible' ), 'Second audit must preserve the File 00 production blocker truth.' );
$assert( str_contains( $audit, 'Hostinger staging acceptance' ), 'Second audit must keep staging distinct from source/CI closure.' );
$assert( str_contains( $audit, 'Baseline Integrity' ) && str_contains( $audit, 'Final Release Candidate' ), 'Second audit must record exact-head four-workflow closure evidence.' );
