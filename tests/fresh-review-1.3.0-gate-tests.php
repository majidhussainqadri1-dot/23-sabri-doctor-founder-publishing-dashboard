...
foreach ( array( 'Version 1.3.0', '2026-08-07', 'Review B', 'Run #275', 'zero known unresolved blocker/critical', 'Hostinger staging', 'final branch head' ) as $marker ) {
...
$assert( false !== strpos( $review_b, '1f98e4121e5cc71c54f1436ceb852255389485e2' ), 'Review B must identify the exact reviewed head.' );
$assert( false !== stripos( $review_b, 'conclusion **success**' ), 'Review B must identify successful exact-head automated evidence before its independent conclusion.' );
...
$full_plan = (string) file_get_contents( $root . '/tests/full-plan-completion-tests.php' );
$assert( false !== strpos( $full_plan, 'Version:     1.3.0' ), 'Review A 1.3.0 release-identity regression correction must remain enforced.' );
$assert( false !== strpos( $full_plan, 'Search, Discovery and Ranking' ), 'Current File 26 ownership regression must remain enforced.' );
