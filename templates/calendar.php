...
<section class="spdb-review-calendar" aria-labelledby="<?php echo esc_attr( $title_id ); ?>">
	...
	<div class="spdb-review-calendar-summary" aria-label="<?php esc_attr_e( 'Calendar summary', 'sabri-publishing-dashboard' ); ?>">
		<article><span><?php esc_html_e( 'Providers', 'sabri-publishing-dashboard' ); ?></span><strong><?php echo esc_html( (string) $calendar_result['provider_count'] ); ?></strong></article>
		<article><span><?php esc_html_e( 'Validated entries', 'sabri-publishing-dashboard' ); ?></span><strong><?php echo esc_html( (string) $calendar_result['validated_count'] ); ?></strong></article>
	...
	<?php if ( empty( $calendar_result['items'] ) ) : ?>
		<p class="spdb-empty-state"><?php esc_html_e( 'No validated native calendar entry is available for the current scope.', 'sabri-publishing-dashboard' ); ?></p>
	<?php else : ?>
			...
			<table class="spdb-review-calendar-table">
				<thead>
					<tr>
						...
						<th><?php esc_html_e( 'Scheduled UTC', 'sabri-publishing-dashboard' ); ?></th>
						<th><?php esc_html_e( 'Native timezone', 'sabri-publishing-dashboard' ); ?></th>
						<th><?php esc_html_e( 'Status', 'sabri-publishing-dashboard' ); ?></th>
						<th><?php esc_html_e( 'Conflicts', 'sabri-publishing-dashboard' ); ?></th>
				...
				<?php foreach ( $calendar_result['items'] as $item ) : ?>
					<tr>
						...
						<td>
							<a class="spdb-button spdb-button--secondary" href="<?php echo esc_url( $item['native_edit_url'] ); ?>"><?php esc_html_e( 'Open Native Schedule', 'sabri-publishing-dashboard' ); ?></a>
	...
	<?php endif; ?>
	<p class="spdb-caption"><?php echo esc_html( sprintf( __( 'Generated at %s. Native schedule state and cron reconciliation remain authoritative.', 'sabri-publishing-dashboard' ), $calendar_result['generated_at_gmt'] ) ); ?></p>
</section>
