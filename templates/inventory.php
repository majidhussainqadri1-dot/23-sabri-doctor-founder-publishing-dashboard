...
<section class="spdb-inventory" aria-labelledby="<?php echo esc_attr( $title_id ); ?>">
	...
	<form class="spdb-inventory-filter" method="get" action="<?php echo esc_url( home_url( '/' . SPDB_Dashboard_Router::ROUTE . '/' ) ); ?>">
		<input type="hidden" name="view" value="inventory">
		<div class="spdb-form-grid">
			<label>
				<span><?php esc_html_e( 'Search', 'sabri-publishing-dashboard' ); ?></span>
				<input type="search" name="search" value="<?php echo esc_attr( (string) $query['search'] ); ?>" maxlength="100" autocomplete="off">
			</label>
			<label>
				...
				<select name="provider">
					...
					<?php foreach ( $inventory_providers as $provider_key => $provider ) : ?>
						<option value="<?php echo esc_attr( $provider_key ); ?>" <?php selected( $selected_provider, $provider_key ); ?>>
							<?php echo esc_html( $provider['label'] . ' — ' . $provider['state'] ); ?>
						</option>
					<?php endforeach; ?>
				</select>
			</label>
			<label>
				<span><?php esc_html_e( 'Object type', 'sabri-publishing-dashboard' ); ?></span>
				<input type="text" name="object_type" value="<?php echo esc_attr( $selected_type ); ?>" maxlength="64" pattern="[a-z0-9_-]*" autocomplete="off">
			</label>
			<label>
				<span><?php esc_html_e( 'Lifecycle', 'sabri-publishing-dashboard' ); ?></span>
				<select name="lifecycle_state">
					<option value=""><?php esc_html_e( 'Any lifecycle state', 'sabri-publishing-dashboard' ); ?></option>
					<?php foreach ( SPDB_Projection_Validator::lifecycle_states() as $state ) : ?>
						<option value="<?php echo esc_attr( $state ); ?>" <?php selected( (string) $query['lifecycle_state'], $state ); ?>><?php echo esc_html( ucwords( str_replace( '_', ' ', $state ) ) ); ?></option>
					<?php endforeach; ?>
				</select>
			</label>
			<label>
				...
				<select name="review_state">
					<option value=""><?php esc_html_e( 'Any review state', 'sabri-publishing-dashboard' ); ?></option>
					<?php foreach ( SPDB_Projection_Validator::review_states() as $state ) : ?>
						<option value="<?php echo esc_attr( $state ); ?>" <?php selected( (string) $query['review_state'], $state ); ?>><?php echo esc_html( ucwords( str_replace( '_', ' ', $state ) ) ); ?></option>
					<?php endforeach; ?>
				</select>
			</label>
			<label>
				<span><?php esc_html_e( 'Visibility', 'sabri-publishing-dashboard' ); ?></span>
				<select name="visibility_state">
					<option value=""><?php esc_html_e( 'Any visibility', 'sabri-publishing-dashboard' ); ?></option>
					<?php foreach ( SPDB_Projection_Validator::visibility_states() as $state ) : ?>
						<option value="<?php echo esc_attr( $state ); ?>" <?php selected( (string) $query['visibility_state'], $state ); ?>><?php echo esc_html( ucwords( str_replace( '_', ' ', $state ) ) ); ?></option>
					<?php endforeach; ?>
				</select>
			</label>
			<label>
				...
				<select name="sort">
					<?php foreach ( array( 'modified_at' => __( 'Last modified', 'sabri-publishing-dashboard' ), 'created_at' => __( 'Created date', 'sabri-publishing-dashboard' ), 'scheduled_at' => __( 'Scheduled date', 'sabri-publishing-dashboard' ), 'published_at' => __( 'Published date', 'sabri-publishing-dashboard' ), 'title' => __( 'Title', 'sabri-publishing-dashboard' ) ) as $sort_key => $sort_label ) : ?>
						<option value="<?php echo esc_attr( $sort_key ); ?>" <?php selected( (string) $query['sort'], $sort_key ); ?>><?php echo esc_html( $sort_label ); ?></option>
					<?php endforeach; ?>
				</select>
			</label>
			<label>
				<span><?php esc_html_e( 'Direction', 'sabri-publishing-dashboard' ); ?></span>
				<select name="direction">
					<option value="desc" <?php selected( (string) $query['direction'], 'desc' ); ?>><?php esc_html_e( 'Descending', 'sabri-publishing-dashboard' ); ?></option>
					<option value="asc" <?php selected( (string) $query['direction'], 'asc' ); ?>><?php esc_html_e( 'Ascending', 'sabri-publishing-dashboard' ); ?></option>
				</select>
			</label>
			<?php if ( 'founder' === $workspace['key'] ) : ?>
				<label>
					<span><?php esc_html_e( 'Scope', 'sabri-publishing-dashboard' ); ?></span>
					<select name="scope">
						<option value="own" <?php selected( (string) $query['scope'], 'own' ); ?>><?php esc_html_e( 'My content', 'sabri-publishing-dashboard' ); ?></option>
						<option value="institution" <?php selected( (string) $query['scope'], 'institution' ); ?>><?php esc_html_e( 'Institution-wide', 'sabri-publishing-dashboard' ); ?></option>
					</select>
				</label>
			<?php endif; ?>
			<label>
				<span><?php esc_html_e( 'Items per page', 'sabri-publishing-dashboard' ); ?></span>
				<select name="per_page">
					<?php foreach ( array( 10, 20, 30, 50 ) as $count ) : ?>
						<option value="<?php echo esc_attr( (string) $count ); ?>" <?php selected( (int) $query['per_page'], $count ); ?>><?php echo esc_html( (string) $count ); ?></option>
					<?php endforeach; ?>
				</select>
			</label>
		</div>
	...
	<?php else : ?>
		<p class="spdb-caption">
			<?php echo esc_html( sprintf( __( '%1$d native items reported across %2$d queried providers. Scope: %3$s.', 'sabri-publishing-dashboard' ), (int) $inventory_result['total'], count( $inventory_result['providers_queried'] ), (string) $inventory_result['scope'] ) ); ?>
		</p>

		<?php if ( ! empty( $inventory_result['provider_errors'] ) ) : ?>
			<div class="spdb-notice spdb-notice--warning" role="status">
				<strong><?php esc_html_e( 'Partial provider results', 'sabri-publishing-dashboard' ); ?></strong>
				<ul>
					<?php foreach ( $inventory_result['provider_errors'] as $provider_error ) : ?>
						<li><?php echo esc_html( (string) $provider_error['provider_key'] . ': ' . (string) $provider_error['code'] ); ?></li>
					<?php endforeach; ?>
				</ul>
			</div>
		<?php endif; ?>
		...
		<?php if ( empty( $inventory_result['items'] ) ) : ?>
			<div class="spdb-empty-state" role="status">
				<?php esc_html_e( 'No validated native content projections match this query. This zero state is not replaced with fabricated content.', 'sabri-publishing-dashboard' ); ?>
			</div>
		<?php else : ?>
			<div class="spdb-table-wrap" role="region" aria-label="<?php esc_attr_e( 'Federated content inventory results', 'sabri-publishing-dashboard' ); ?>" tabindex="0">
				<table class="spdb-inventory-table">
					<thead>
						<tr>
							<th><?php esc_html_e( 'Content', 'sabri-publishing-dashboard' ); ?></th>
							<th><?php esc_html_e( 'Provider / Type', 'sabri-publishing-dashboard' ); ?></th>
							<th><?php esc_html_e( 'Lifecycle', 'sabri-publishing-dashboard' ); ?></th>
							<th><?php esc_html_e( 'Review', 'sabri-publishing-dashboard' ); ?></th>
							<th><?php esc_html_e( 'Visibility', 'sabri-publishing-dashboard' ); ?></th>
							<th><?php esc_html_e( 'Modified', 'sabri-publishing-dashboard' ); ?></th>
							<th><?php esc_html_e( 'Inspector', 'sabri-publishing-dashboard' ); ?></th>
						</tr>
					</thead>
					<tbody>
					<?php foreach ( $inventory_result['items'] as $item ) : ?>
						...
						$inspect_url = add_query_arg(
							array(
								'view'             => 'inventory',
								'inspect_provider' => $item['provider_key'],
								'inspect_type'     => $item['object_type'],
								'inspect_id'       => $item['object_id'],
							),
							home_url( '/' . SPDB_Dashboard_Router::ROUTE . '/' )
						);
						?>
						<tr>
		...
		<?php if ( (int) $inventory_result['pages'] > 1 ) : ?>
			<nav class="spdb-pagination" aria-label="<?php esc_attr_e( 'Inventory pages', 'sabri-publishing-dashboard' ); ?>">
				<?php if ( (int) $inventory_result['page'] > 1 ) : ?>
					<a class="spdb-button spdb-button--secondary" href="<?php echo esc_url( add_query_arg( array( 'view' => 'inventory', 'page' => (int) $inventory_result['page'] - 1, 'per_page' => (int) $inventory_result['per_page'] ), home_url( '/' . SPDB_Dashboard_Router::ROUTE . '/' ) ) ); ?>"><?php esc_html_e( 'Previous', 'sabri-publishing-dashboard' ); ?></a>
				<?php endif; ?>
				<span><?php echo esc_html( sprintf( __( 'Page %1$d of %2$d', 'sabri-publishing-dashboard' ), (int) $inventory_result['page'], (int) $inventory_result['pages'] ) ); ?></span>
				<?php if ( (int) $inventory_result['page'] < (int) $inventory_result['pages'] && ( (int) $inventory_result['page'] + 1 ) * (int) $inventory_result['per_page'] <= SPDB_Inventory_Query::MAX_WINDOW ) : ?>
					<a class="spdb-button spdb-button--secondary" href="<?php echo esc_url( add_query_arg( array( 'view' => 'inventory', 'page' => (int) $inventory_result['page'] + 1, 'per_page' => (int) $inventory_result['per_page'] ), home_url( '/' . SPDB_Dashboard_Router::ROUTE . '/' ) ) ); ?>"><?php esc_html_e( 'Next', 'sabri-publishing-dashboard' ); ?></a>
				<?php endif; ?>
			</nav>
		<?php endif; ?>
	<?php endif; ?>
		...
		<section class="spdb-inspector" aria-labelledby="<?php echo esc_attr( $instance_id . '-inspector-title' ); ?>">
			...
			<?php else : ?>
				...
				<?php if ( ! empty( $inventory_item['destinations'] ) ) : ?>
					<div class="spdb-destination-actions" aria-label="<?php esc_attr_e( 'Native destinations', 'sabri-publishing-dashboard' ); ?>">
						<?php foreach ( array( 'edit' => __( 'Continue Editing', 'sabri-publishing-dashboard' ), 'preview' => __( 'Preview', 'sabri-publishing-dashboard' ), 'public' => __( 'View Public Page', 'sabri-publishing-dashboard' ) ) as $destination_key => $destination_label ) : ?>
							<?php if ( ! empty( $inventory_item['destinations'][ $destination_key ] ) ) : ?>
								<a class="spdb-button<?php echo 'edit' === $destination_key ? '' : ' spdb-button--secondary'; ?>" href="<?php echo esc_url( $inventory_item['destinations'][ $destination_key ] ); ?>" rel="noopener noreferrer"><?php echo esc_html( $destination_label ); ?></a>
							<?php endif; ?>
						<?php endforeach; ?>
					</div>
				<?php endif; ?>
				...
				<?php if ( ! empty( $inventory_item['allowed_operations'] ) ) : ?>
					...
					<ul>
						<?php foreach ( $inventory_item['allowed_operations'] as $operation ) : ?>
							<li><code><?php echo esc_html( $operation['key'] ); ?></code> — <?php echo $operation['environment_eligible'] ? esc_html__( 'environment gate eligible; execution still not exposed', 'sabri-publishing-dashboard' ) : esc_html__( 'not accepted for environment writes', 'sabri-publishing-dashboard' ); ?></li>
						<?php endforeach; ?>
					</ul>
				<?php endif; ?>
				...
				<?php if ( ! empty( $inventory_item['compliance_alerts'] ) ) : ?>
					...
					<ul class="spdb-compliance-list">
						<?php foreach ( $inventory_item['compliance_alerts'] as $alert ) : ?>
							<li class="spdb-notice spdb-notice--<?php echo esc_attr( $alert['level'] ); ?>"><?php echo esc_html( $alert['message'] ); ?></li>
						<?php endforeach; ?>
					</ul>
				<?php endif; ?>
			<?php endif; ?>
